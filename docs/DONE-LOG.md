...
**Open, carried forward:** the **"why" dies at the commit boundary** — the brief's per-change rationale has no home (transient brief + learnings forbidden from restating changes + ops-only payload), recorded as a fork in [`docs/open-questions.md`](open-questions.md) to ride with ADR-0021 Amds 1+2. Also still open: **Amd1-OQ2** (brief discarded vs. durable — the lean is now contested by that fork), **OQ3** (compare: commit or advisory), **OQ5** (do the in-app workbench verbs survive), and OQ6's non-gating **Claude portability run**.
