...
Last updated: July 23, 2026 (**Live dispatch target = [ADR-0043](decisions/ADR-0043-model-call-chokepoint.md) load test — three advisory verbs exercise the new record before S3.** Separately, **[ADR-0021](decisions/ADR-0021-recipe-variations.md) Amds 1 + 2 are ratified and Amd 3 is Accepted (2026-07-23)** — variations are now **scoped and dispatchable** as **V1 + V2** (schema-free) and **V3** (one synced table + its reader, answering the "why" fork as a recipe-scoped deliberation log). **Ratification is not promotion: the live target above is unchanged, and whether the two arcs run in parallel is Jon's open call.**) Completed-slice history and strategic background live in [`docs/DONE-LOG.md`](DONE-LOG.md).
...
- **Variations are half-built, and it shows in daily use — now RATIFIED and dispatchable (2026-07-23), still not the live target.** [ADR-0021 **Amds 1 + 2**](decisions/ADR-0021-recipe-variations.md) are **Accepted** (hand-edit through the resolved view with ops **derived** on save; split-off and promote-to-base), and **Amd 3** answers the **"why" fork** as option **(d)** — a recipe-scoped deliberation log. Until V1 ships, a variation is still a display-time overlay that every read folds and no write understands; the interim guard (editor notice + hand-off confirmation) only *says so*. **⚠️ Ratification is not promotion. This lost the slot to ADR-0043 on 2026-07-23 — Jon's explicit call, made with the daily-use cost in view — and ratifying it later the same day did not change that.** It is now a fully scoped **Ready Effort** (V1+V2, then V3) and the natural successor once the chokepoint arc lands. **Running it in parallel with the ADR-0043 load test is a live question Jon has not answered — do not infer it, and do not quietly promote it.** *(If it is ever run in parallel, the two arcs are near-disjoint: V1–V3 make **no model calls at all**, so they never touch `ModelCall`; the only shared file is `RecipeAdjustment.swift`, whose derivation half is untouched by ADR-0043, plus a regeneratable `project.pbxproj`. The real contention is this file and Jon's review bandwidth, not the code.)*
