...
Last updated: July 16, 2026 (ADR-0039 **Amendment 2 — Peek detent dropped**: the Playbook resize handle now cycles **Comfortable ↔ Wide** only; the degenerate min-width sliver is gone, and the toolbar Show/Hide button owns the honest hide. `RecipePlaybookColumnDetent` is now `comfortable · wide`, the enum-count width math auto-rebalances (**Comfortable → ½ max, Wide → full max**), and a persisted `"peek"` preference falls back to Comfortable through the existing nil-coalescing getter — no migration. Architect-approved + local `generic/platform=iOS` build gate green, PR #196 → DONE-LOG; Jon device pass pending. **Next Up is now open — Jon picks the next effort; Slice D (menu adopts the Playbook column) stays parked.**)
...
**No live dispatch target — Jon picks the next effort.** The Peek-detent drop (the prior Next Up) shipped in PR #196 → DONE-LOG. Per the intake rule, **do not infer the next task** — dispatch only what Jon names here. Standing candidates are the **Feature efforts** list just below and the **Ready Efforts** queue; **Slice D (menu adopts the Playbook column) stays parked** until Jon calls it.
