...
**ONE live dispatch target: [ADR-0032](decisions/ADR-0032-workbench-reference-material-fetch.md) S1 — the workbench reference-material model + reduce/cache/store core.**
Dispatch with *"Do the **ADR-0032 S1** effort in `docs/CURRENT_HANDOFF.md`."* If this section is
empty or missing, **STOP and ask Jon — never infer.** See `docs/AGENTS.md` § Work Intake & Dispatch.

**Promoted 2026-07-26 on the standing instruction** — ADR-0032 was demoted for the ferry track, *not* for a
problem, and the ferry track is now closed (all four dispatches shipped; see
[`DONE-LOG`](DONE-LOG.md)). Its full scope is in **Ready Efforts below** — read that entry, then the ADR
**including Amendment 1**, whose two resolutions revise the original Decision and which S1 depends on
([`AGENTS.md`](AGENTS.md) rule 4: the handoff points, it does not duplicate).

**⚠️ S1 adds synced schema.** `WorkbenchReference` is a **synced** record type (entry *and* reduced extract —
OQ2); register it in [`CloudSync.swift`](YesChefPackage/Sources/YesChefCore/CloudSync.swift) **and add it to
the prod-promotion list below in that same PR** ([[handoff-bump-rides-in-slice-pr]]). A synced table is cheap
([[synced-table-cost-calibration]]) — the cost is forgetting to register it before the prod cut.

**Verification: package-only.** `swift build` + `scripts/check-drift.sh` + unit tests against a **stubbed**
fetch client (fixture HTML → asserted reduced text; no network in CI). **No app build and no device pass — S1
adds no UI.** Do not let the companion dispatch below drag an app build onto this one; they are separate PRs.

**Immediately after S1, one small app-layer dispatch — already scoped, do not re-scope it.** Two things that
belong together because both are app-layer, both want the same device pass, and neither earns a PR alone:
...
2. **The Dogfood ferry Dispatch 3 polish bundle.** Jon's device pass on PR
   ...
   finding + six review nits). No schema. **One commit, folded into that PR.**
...
**[ADR-0032](decisions/ADR-0032-workbench-reference-material-fetch.md) — the workbench reference-material fetch (scoped 2026-07-25, demoted 2026-07-26, **S1 promoted back to Next Up the same day** when the ferry track closed).** It lost the slot to appetite, **not** because anything about it changed. **S1 is the live dispatch target — its scope is below; S2 and S3 stay queued here.**
