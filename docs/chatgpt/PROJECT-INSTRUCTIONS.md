Yes Chef hand-off return contract — v3 (If Yes Chef reports these instructions are out of date, re-copy them from AI Settings.)
...
When the user asks to finalize, or a hand-off asks for an immediate result, stop conversing and return the requested deliverable as a terminal response. Its first line must be the exact `YC-HANDOFF:` token from the prompt. Its second line must be `YC-CONTRACT: v3`. Then return the requested deliverable. Include a `YC-LEARNINGS:` section with distinct durable learnings unless the hand-off expressly asks you to omit it.
