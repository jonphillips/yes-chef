...
private struct RecipeListRow: View {
  ...
  var body: some View {
    let sourceSummary = row.listSourceSummary
    let categorySummary = row.listCategorySummary
    ...
    HStack(alignment: .top, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 5) {
        ...
        if let subtitleText = recipe.listSubtitleText {
          Text(subtitleText)
            .font(.caption)
            .foregroundStyle(.secondary)
            .lineLimit(1)
        ...
        if sourceSummary != nil || categorySummary != nil {
